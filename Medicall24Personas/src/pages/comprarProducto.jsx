...
const StepWizard = () => {
  ...
  const Finally = lazy(() => import("../components/purchase/LastView"));
...
const fetchAliados = async () => {
  const url =
    `https://ukvjbkxaqzltngpbrdfi.supabase.co/rest/v1/aliado_comercial?${product.id_aliado != null ? `id_aliado=eq.${product.id_aliado}&select=*` : 'select=*'}`;
  try {
    ...
    setAliados(response.data);
    console.log("aliados:", response.data);
  ...
  const setProduct = (product) => {
    if (product.id_producto_pago) {
      setProductInfo({
        id_producto: product.id_producto_pago,
  ...
  const validateFormData = () => {
    ...
    if (type === "NEQUI") {
      return (
        formData.phoneNumber && localFormData.phone && localFormData.address
      );
    }
  ...
  const updateJsonWithPayload = (payload) => {
    ...
    const updatedJson = {
      ...
      nombre_comprador: `${payload.user?.name1 || ""} ${
        payload.user?.name2 || ""
      } ${payload.user?.lastName1 || ""} ${
        payload.user?.lastName2 || ""
      }`.trim(),
      email_comprador: payload.user?.email || null,
      id_usuario_medicall: payload.user?.id || null
    };
  ...
  const updateJsonWithOrder = (trId, paymentMethod, status, description, ip) => {

    const templatejson = registerTemplate;
    // Plantilla inicial del JSON
    if (trId && paymentMethod && status) {
      console.log("Información de pago obtenida:", trId, paymentMethod, status, description, ip);
      // Aquí defines cómo mapear los datos del payload al JSON
      const updatedJson = {
        ...templatejson,
        id_transaccion: trId,
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://medicallapi-test.azurewebsites.net/api/Patients",
        payload
      );
  ...
  const getOrder = async (transactionId) => {
      try {
        // Realiza la solicitud al endpoint
        const statusResponse = await axios.get(
          `${BASE_URL}/GetStatusOrder/${transactionId}`
        );
        ...
        if (trId && paymentMethod && status) {
          const order = updateJsonWithOrder(trId, paymentMethod, status, description, ip);
  ...
  const handleSubmit = async (order) => {
    ...
    try {
      const response = await axios.post(
        'https://ukvjbkxaqzltngpbrdfi.supabase.co/rest/v1/registro_compra',
        order,
        {
          headers: {
            apikey: import.meta.env.VITE_SUPABASE_CLIENT_ANON_KEY_API,
    ...
    <main>
      ...
      >
        ...
        <div className={`${stepClass} relative overflow-hidden`}>
          ...
          >
            <StepTwo payload={payload} getDepartamentos={getDepartamentos} />
          </div>
          {/* Paso 3 */}
          <div
            className={`absolute top-0 left-0 w-full transition-all duration-500 transform ${
